...
-   **비연결**(Connectionless)
    ...
    -   여러 사용자가 요청할 시 각각 사용자의 요청을 구분할 수 없어 제대로 된 응답 데이터를 전송할 수 없다는 단점
-   **무상태**(Stateless)
...
  대칭키(공개키) 암호화와 비대칭키(세션키) 암호화를 모두 사용하여 빠른 연산 속도와 안정성을 갖고 있다.
...
  HTTP는 암호화 과정 없이 데이터를 전달하기 때문에 보안에 취약한 반면, HTTPS는 암호화로 네트워크 상에서 열람 및 수정이 불가능해 안전하다.
  ...
  또한, HTTP는 비연결형으로 웹 페이지를 보던 중 인터넷 연결이 끊겼다 다시 연결되어도 페이지를 계속 볼 수 있지만 HTTPS의 경우에는 소켓 자체에서 인증 과정이 있기 때문에 인터넷 연결이 끊기면 소켓도 끊어져 재인증하는 시간이 소요된다. 
